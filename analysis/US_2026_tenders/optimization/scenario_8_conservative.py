...
Same as Scenario 7 but with a $5M undiscounted FedEx threshold instead of $4.5M.
This builds in a ~$500K safety buffer to protect against seasonal volume fluctuations
pushing FedEx below the 16% earned discount tier.

Dual-method approach (same as S7):
...
S7's "Drop OnTrac" variant met the $4.5M threshold by just $16.
S8 tests whether the same strategy holds with $500K more headroom.
"""
...
FEDEX_UNDISCOUNTED_THRESHOLD = 5_000_000  # $5M conservative buffer (vs $4.5M in S7)
BAKED_FACTOR = 1 - PP_DISCOUNT - BAKED_EARNED  # 0.37
FEDEX_BASE_RATE_THRESHOLD = FEDEX_UNDISCOUNTED_THRESHOLD * BAKED_FACTOR  # $1,850,000

# S6 variants (without P2P) — same as S7
S6_VARIANTS = [
...
def run_s8_variant(df: pl.DataFrame, s8_variant: dict, s6_variant: dict) -> dict:
    """Run a S8 variant using dual-method approach."""
    name = s8_variant["name"]
    minimums = s8_variant["minimums"]

    print(f"\n  {'=' * 66}")
    print(f"  {name} ({s8_variant['short']}): carriers = {', '.join(s8_variant['available'])}")
    if minimums:
        print(f"  Minimums: {', '.join(f'{k} >= {v:,}' for k, v in minimums.items())}")
    else:
        print(f"  Minimums: none")
    print(f"  FedEx threshold: ${FEDEX_UNDISCOUNTED_THRESHOLD:,.0f} undiscounted (conservative)")
    print(f"  {'=' * 66}")

    # Run S6 baseline (with S8's higher threshold)
    print(f"\n    Running S6 baseline ({s6_variant['short']}) with $5M threshold...")
    ...
    print(f"\n    Method A: Greedy with P2P...")
    result_a = method_a_greedy(df, s8_variant)
...
def main():
    print("=" * 70)
    print("Scenario 8: Conservative P2P + FedEx 16% ($5M buffer)")
    print(f"FedEx rates adjusted: earned discount 18% -> {FEDEX_TARGET_EARNED:.0%}")
    print(f"FedEx threshold: ${FEDEX_UNDISCOUNTED_THRESHOLD:,.0f} undiscounted (conservative)")
    print(f"  vs S7 threshold: $4,500,000")
    print("=" * 70)
    ...
    # Run all S8 variants
    print("\n[3] Running S8 optimization variants...")
    results = {}
    for s8_var, s6_var in zip(S8_VARIANTS, S6_VARIANTS):
        result = run_s8_variant(df, s8_var, s6_var)
        results[s8_var["short"]] = result
    ...
    print("=" * 120)
    print(f"\n{'Variant':<30} {'Short':<8} {'S8 Cost':>14} {'S6 Cost':>14} {'vs S6':>12} {'vs S1':>14} {'FedEx Tier':>12} {'Method':>8}")
    print("-" * 116)
    print(f"{'S1 Baseline (16%)':<30} {'-':<8} {'-':>14} {'-':>14} {'-':>12} {'-':>14} {'-':>12} {'-':>8}")

    for s8_var in S8_VARIANTS:
        r = results[s8_var["short"]]
        s8_cost = r["total_cost"]
        s6_cost = r["s6_cost"]
        vs_s6 = s6_cost - s8_cost
        vs_s1 = SCENARIO_1_BASELINE - s8_cost
        vs_s1_pct = vs_s1 / SCENARIO_1_BASELINE * 100
        tier = "MET" if r["fedex_threshold_met"] else "NOT MET"
        print(
            f"{r['name']:<30} {r['short']:<8} "
            f"${s8_cost:>13,.0f} ${s6_cost:>13,.0f} "
            f"${vs_s6:>+11,.0f} "
    ...
    # S8 <= S6 guarantee check
    print("\n### S8 <= S6 Guarantee Check")
    for s8_var in S8_VARIANTS:
        r = results[s8_var["short"]]
        ok = r["total_cost"] <= r["s6_cost"] + 0.01
        status = "OK" if ok else "VIOLATION"
        print(f"    {r['name']}: S8=${r['total_cost']:,.2f} vs S6=${r['s6_cost']:,.2f} - {status}")

    # ====================
    # DETAILED: DROP ONTRAC (key variant)
    # ====================
    key_variant = results["UFP"]
    print(f"\n{'=' * 90}")
    print(f"DETAILED: Drop OnTrac + P2P ($5M conservative threshold)")
    print(f"{'=' * 90}")
    ...
    for carrier in ["USPS"]:
        if carrier in key_variant["minimums"]:
            ...
            status = "MET" if vol >= min_req else "SHORT"
    ...
    print(f"    FedEx headroom: ${fedex_undiscounted - FEDEX_UNDISCOUNTED_THRESHOLD:+,.0f} above threshold")

    # ====================
    # ALSO DETAIL: BOTH CONSTRAINTS
    # ====================
    both = results["OUFP"]
    print(f"\n{'=' * 90}")
    print(f"DETAILED: Both Constraints + P2P ($5M conservative)")
    ...
    # Save "Drop OnTrac" assignments
    output_dir = Path("analysis/US_2026_tenders/results/scenario_8_conservative")
