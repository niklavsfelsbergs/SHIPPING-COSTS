"""
Demand Additional Handling Surcharge (DEM_AHS)

Seasonal surcharge applied when AHS (Dimensions or Weight) triggers during peak period.

Two pricing tiers:
- Phase 1 (Sep 29 - Nov 23): $4.13
- Phase 2 (Nov 24 - Jan 18): $5.45

Triggers when:
- surcharge_ahs OR surcharge_ahs_weight is True
- ship_date is within demand period
"""

import polars as pl
from shared.surcharges import Surcharge, in_period


class DEM_AHS(Surcharge):
    """Demand Additional Handling - seasonal surcharge when AHS applies."""

    name = "DEM_AHS"

    # -------------------------------------------------------------------------
    # PRICING
    # -------------------------------------------------------------------------
    # Dummy values - actual cost is calculated in cost() method
    list_price = 4.13
    discount = 0.0

    # Phase 1 and Phase 2 prices
    PHASE_1_PRICE = 4.13  # Sep 29 - Nov 23
    PHASE_2_PRICE = 5.45  # Nov 24 - Jan 18

    # -------------------------------------------------------------------------
    # DEPENDENCIES
    # -------------------------------------------------------------------------
    depends_on = "AHS"  # Also triggered by AHS_Weight
    period_start = (9, 29)   # Sep 29
    period_end = (1, 18)     # Jan 18

    # Phase 2 boundaries (peak of peak)
    PHASE_2_START = (11, 24)  # Nov 24
    PHASE_2_END = (1, 18)     # Jan 18

    @classmethod
    def conditions(cls) -> pl.Expr:
        """
        Triggers when AHS (Dimensions or Weight) flag is set and within demand period.

        Note: Both AHS-Dimensions and AHS-Weight can trigger demand AHS charge.
        Only one of them can be True due to exclusivity group.
        """
        has_ahs = pl.col("surcharge_ahs") | pl.col("surcharge_ahs_weight")
        in_demand_period = in_period(cls.period_start, cls.period_end)
        return has_ahs & in_demand_period

    @classmethod
    def cost(cls) -> pl.Expr:
        """
        Return surcharge cost based on phase.

        Phase 1 (Sep 29 - Nov 23): $4.13
        Phase 2 (Nov 24 - Jan 18): $5.45
        """
        in_phase_2 = in_period(cls.PHASE_2_START, cls.PHASE_2_END)

        return (
            pl.when(in_phase_2)
            .then(pl.lit(cls.PHASE_2_PRICE))
            .otherwise(pl.lit(cls.PHASE_1_PRICE))
        )
