...
st.set_page_config(page_title="Accuracy | USPS", layout="wide")
...
st.caption("Detection accuracy for deterministic surcharges (NSL1, NSL2). NSV is compared separately.")
...
for surcharge in DETERMINISTIC_SURCHARGES:
    ...
    if flag_col not in df.columns or actual_col not in df.columns:
        continue
        ...
        if len(fp_df) > 0 or len(fn_df) > 0:
            with st.expander(f"{surcharge.upper()} - FP: {len(fp_df):,} | FN: {len(fn_df):,}"):
                ...
                if len(fp_df) > 0:
                    ...
                    st.dataframe(fp_df.select([c for c in detail_cols if c in fp_df.columns]).head(20), use_container_width=True, hide_index=True)
                    csv_fp = fp_df.select([c for c in detail_cols if c in fp_df.columns]).to_pandas().to_csv(index=False)
                ...
                if len(fn_df) > 0:
                    ...
                    st.dataframe(fn_df.select([c for c in detail_cols if c in fn_df.columns]).head(20), use_container_width=True, hide_index=True)
                    csv_fn = fn_df.select([c for c in detail_cols if c in fn_df.columns]).to_pandas().to_csv(index=False)
...
# NSV / Noncompliance comparison
st.markdown("**NSV (Noncompliance) Comparison**")
st.caption("NSV (nonstandard volume) maps to 'noncompliance' on invoices.")

nsv_expected = df["cost_nsv"].fill_null(0) > 0
nsv_actual = df["actual_noncompliance"].fill_null(0) > 0

tp_nsv = int((nsv_expected & nsv_actual).sum())
fp_nsv = int((nsv_expected & ~nsv_actual).sum())
fn_nsv = int((~nsv_expected & nsv_actual).sum())

precision_nsv = (tp_nsv / (tp_nsv + fp_nsv) * 100) if (tp_nsv + fp_nsv) > 0 else 100.0
recall_nsv = (tp_nsv / (tp_nsv + fn_nsv) * 100) if (tp_nsv + fn_nsv) > 0 else 100.0
...
nsv_row = [{
    ...
    "False Pos": fp_nsv,
    "False Neg": fn_nsv,
    "Precision": f"{precision_nsv:.1f}%",
    "Recall": f"{recall_nsv:.1f}%",
}]
st.dataframe(pl.DataFrame(nsv_row), use_container_width=True, hide_index=True)
...
if len(valid_weight) > 0:
    ...
    default_hi = min(20.0, max_w)  # USPS max is 20 lbs
