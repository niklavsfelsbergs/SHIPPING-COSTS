...
Calculates hypothetical USPS shipping costs for ALL US shipments
(not just USPS shipments) and uploads them to the database.

This enables carrier cost optimization analysis by comparing what shipments
would cost with USPS vs what they actually cost with their real carrier.
...
Usage:
    python -m carriers.usps.scripts.upload_expected_all_us --full
    python -m carriers.usps.scripts.upload_expected_all_us --incremental
    python -m carriers.usps.scripts.upload_expected_all_us --days 7
    python -m carriers.usps.scripts.upload_expected_all_us --full --dry-run
    python -m carriers.usps.scripts.upload_expected_all_us --full --parquet --start-date 2025-01-01 --end-date 2025-12-31
    ...
    python -m carriers.usps.scripts.upload_expected_all_us --full --parquet-data shared/data/pcs_shipments_all_us_2025-01-01_2025-12-31.parquet --start-date 2025-01-01 --end-date 2025-12-31
"""
...
from carriers.usps.data import load_pcs_shipments_all_us
...
COST_COLUMNS_RENAME = {
    "cost_base": "usps_cost_base",
    "cost_nsl1": "usps_cost_nsl1",
    "cost_nsl2": "usps_cost_nsl2",
    "cost_nsv": "usps_cost_nsv",
    "cost_peak": "usps_cost_peak",
    "cost_subtotal": "usps_cost_subtotal",
    "cost_total": "usps_cost_total",
}
...
UPLOAD_COLUMNS = [
    ...
    # Zone (1)
    "shipping_zone",
    # Cost positions (7) - carrier-prefixed
    "usps_cost_base", "usps_cost_nsl1", "usps_cost_nsl2", "usps_cost_nsv",
    "usps_cost_peak", "usps_cost_subtotal", "usps_cost_total",
    # Metadata (2)
...
def run_pipeline(
    ...
    3. Mark overweight shipments (> 20 lbs for USPS Ground Advantage)
    4. Override production_site to Columbus (CMH zones)
    5. Calculate costs using existing calculator
    6. Set overweight shipments to penalty cost ($200)
    ...
    # 3. Mark overweight shipments (USPS Ground Advantage max 20 lbs)
    ...
    # 6. Set overweight shipments to penalty cost
    df = df.with_columns([
        pl.when(pl.col("_is_overweight"))
        ...
        pl.when(pl.col("_is_overweight"))
        .then(pl.lit(0.0))
        .otherwise(pl.col("cost_nsv"))
        .alias("cost_nsv"),
        pl.when(pl.col("_is_overweight"))
        .then(pl.lit(0.0))
        .otherwise(pl.col("cost_peak"))
        .alias("cost_peak"),
        pl.when(pl.col("_is_overweight"))
        .then(pl.lit(OVERWEIGHT_PENALTY_COST))
...
def run_full_mode(
    ...
    print("=" * 60)
    print("FULL MODE - ALL US EXPECTED COSTS (USPS)")
    print("=" * 60)
...
def run_incremental_mode(
    ...
    print("=" * 60)
    print("INCREMENTAL MODE - ALL US EXPECTED COSTS (USPS)")
    print("=" * 60)
...
def run_days_mode(
    ...
    print("=" * 60)
    print(f"DAYS MODE ({days} days) - ALL US EXPECTED COSTS (USPS)")
    print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description="Upload expected USPS shipping costs for ALL US shipments",
