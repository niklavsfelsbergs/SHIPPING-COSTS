...
Shipping cost calculator for multiple carriers. Calculates expected shipping costs from PCS (production system) shipment data, uploads to Redshift database, and compares against actual invoice costs.
...
This registers `shared` and `carriers` as importable packages. The `-e` flag means "editable" - code changes take effect immediately without reinstalling.
...
pytest carriers/ontrac/tests/test_calculate_costs.py  # Run specific test file
pytest -v                                             # Verbose output
```

### Interactive Calculator
```bash
python -m carriers.ontrac.scripts.calculator    # CLI tool to calculate cost for single shipment
```

### Upload Expected Costs to Database
```bash
python -m carriers.ontrac.scripts.upload_expected --full         # Full recalculation from 2025-01-01
python -m carriers.ontrac.scripts.upload_expected --incremental  # From latest date in DB
python -m carriers.ontrac.scripts.upload_expected --days 7       # Last N days
python -m carriers.ontrac.scripts.upload_expected --dry-run      # Preview without changes
```
...
### Cost Calculator (`carriers/ontrac/calculate_costs.py`)
...
### Surcharges (`carriers/ontrac/surcharges/`)
...
### Data Sources
- `carriers/ontrac/data/loaders/pcs.py` - Loads shipment data from PCS database
- `carriers/ontrac/data/reference/zones.csv` - ZIP code to zone mapping (PHX and CMH production sites)
- `carriers/ontrac/data/reference/base_rates.csv` - Rate card by weight bracket and zone
- `carriers/ontrac/data/reference/fuel.py` - Weekly fuel surcharge rate
- `carriers/ontrac/data/reference/billable_weight.py` - DIM factor configuration
...
- `push_data()` - Upload DataFrame to table (batch size 5000, single commit at end)
...
### Version Stamping
Update `carriers/ontrac/version.py` when changing rates/config. Version is stamped on all calculator output for audit trail.

## Configuration Updates

See `carriers/ontrac/maintenance/README.md` for detailed instructions on updating:
